...
class DQNAgent:
    def __init__(self, env, discount_factor = 0.95, epsilon_greedy = 1, epsilon_min=0.01, epsilon_decay=0.995, learning_rate=1e-3, max_memory_size=2000):
        self.env = env 
...
def plot_learning_history(history):
    ...
    trs = np.array(history)
    plt.plot(episodes, trs, lw=4, marker='o', markersize=10)
    ...
    for e in range(EPISODE):
        ...
        for i in range(500):
            ...
            if done:
                ...
                print('Episode: %d/%d, Total reward: %d' % (e, EPISODE, i))
                break
